...
llama_numa_shard_pair llama_numa_shard_tensor(const ggml_tensor * src, ggml_context * ctx, int axis) {
    llama_numa_shard_pair out;
    GGML_ASSERT(axis == 0 || axis == 1);
    const int64_t n_split   = src->ne[axis];
    const int64_t n_split_s = n_split / 2;
    const size_t  blck      = ggml_blck_size(src->type);
    // the split dim must stay quant-block aligned (contiguous byte rows, block-decodable)
    if (n_split % (2 * blck) != 0 || n_split_s * 2 != n_split) {
        return out;
    }

    // one allocation holding BOTH shards (each half the source): total memory = the source
    void * buf = nullptr;
    ...
    out.axis = axis;
    for (int k = 0; k < 2; k++) {
        int64_t ne[3] = { src->ne[0], src->ne[1], src->ne[2] };
        ne[axis] = n_split_s;
        ggml_tensor * t = ggml_new_tensor_3d(ctx, src->type, ne[0], ne[1], ne[2]);
        t->numa_node = k;
...
// tensor-split NUMA (P1 inc-3): copy the loaded expert tensor's halves into its shards.
//   axis 0 (down, input-split):  shard k = the k-th half of every row   (strided per row)
//   axis 1 (gate/up, output-split): shard k = the k-th half of each expert's rows (contiguous)
void llama_numa_shard_copy(const llama_numa_shard_pair & shards, const ggml_tensor * src, int axis) {
    ...
    for (int k = 0; k < 2; k++) {
        ...
        const uint8_t * sbase = (const uint8_t *) src->data;
        uint8_t * dbase = (uint8_t *) shards.shards[k]->data;
        if (axis == 1) {
            // output-split: split ne[1] (rows). each expert's rows are contiguous in memory, so
            //   each shard is a contiguous half of every expert's block (one memcpy per expert)
            const size_t nb_expert = total / src->ne[2];
            const size_t nb_half   = nb_expert / 2;
            for (int64_t e = 0; e < src->ne[2]; e++) {
                memcpy(dbase + (size_t) e * nb_half,
                       sbase + (size_t) e * nb_expert + (size_t) k * nb_half, nb_half);
            }
        } else {
            // input-split: split ne[0]. the k-th half of every row is a contiguous byte run
            //   (block-aligned); rows are strided across ne[1] x ne[2]
            const size_t row_size   = ggml_row_size(src->type, src->ne[0]);
            const size_t row_size_s = ggml_row_size(src->type, src->ne[0] / 2);
            const int64_t n_rows    = src->ne[1] * src->ne[2];
            const uint8_t * srow = sbase + (size_t) k * row_size_s;
            for (int64_t r = 0; r < n_rows; r++) {
                memcpy(dbase + (size_t) r * row_size_s, srow + (size_t) r * row_size, row_size_s);
            }
        }
    }
}
